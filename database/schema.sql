...
CREATE TABLE IF NOT EXISTS subscription_credits (
    ...
    currency VARCHAR(3) NOT NULL DEFAULT 'USD',
    credit_type VARCHAR(32) NOT NULL DEFAULT 'ADMIN_GRANT', -- RESERVATION_RELEASE, ADMIN_GRANT, PAYMENT_ADJUSTMENT, COMPENSATION
    status VARCHAR(32) NOT NULL DEFAULT 'AVAILABLE', -- AVAILABLE, PARTIALLY_USED, REDEEMED, EXPIRED, CANCELLED
    source_type VARCHAR(64) NULL, -- PAYMENT_TRANSACTION, RESERVATION_RELEASE, ADMIN_MANUAL, PROMOTION
    source_id UUID NULL,
    reference VARCHAR(128) NULL,
    description TEXT NULL,
    expires_at TIMESTAMP WITH TIME ZONE NULL,
    redeemed_transaction_id UUID NULL REFERENCES payment_transactions(id) ON DELETE SET NULL,
    created_by UUID NULL REFERENCES users(id) ON DELETE SET NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
