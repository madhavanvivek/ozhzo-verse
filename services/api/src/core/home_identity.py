...
HOME_ID_ALPHABET = "23456789ABCDEFGHJKLMNPQRSTUVWXYZ"
HOME_ID_PREFIX = "OZH-"
...
async def generate_unique_public_home_id(db: AsyncSession, max_attempts: int = 10) -> str:
    ...
    Checks against the database to guarantee global uniqueness.
    """
    ...
    for _ in range(max_attempts):
        ...
        try:
            ...
            if hasattr(result, "scalar_one_or_none") and result.scalar_one_or_none() is None:
                return candidate_id
            elif hasattr(result, "scalars") and result.scalars().first() is None:
                return candidate_id
        except Exception:
            return candidate_id
